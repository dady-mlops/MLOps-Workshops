...
def push_image_with_retry(image_name, max_retries=5, retry_delay=10):
    """
    Push Docker image to registry with retries
    """
    logger.info(f"Pushing image {image_name} to registry with {max_retries} retries")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if result.returncode == 0:
                logger.info(f"Successfully pushed image on attempt {attempt}")
                return True
            
            logger.warning(f"Push failed (attempt {attempt}/{max_retries}): {result.stderr}")
            
            # Check if we need to re-login to ECR
            if "no basic auth credentials" in result.stderr or "unauthorized" in result.stderr:
                logger.info("Re-authenticating with ECR...")
                # Extract region from image name
                region = image_name.split('.')[3]
                login_to_ecr(region)
            
            if attempt < max_retries:
                time_to_sleep = retry_delay * attempt
                logger.info(f"Waiting {time_to_sleep}s before next attempt...")
                time.sleep(time_to_sleep)
        except Exception as e:
            logger.error(f"Error during push attempt {attempt}: {str(e)}")
            if attempt < max_retries:
